...
> **Versioning note:** published CocoaPods versions lagged this changelog for a while — trunk had only 1.0.0 / 1.1.0 / 1.1.1, with `ios-sdk-v1.1.1` (2026-07-09) actually shipping the 1.4.0-era feature set. Starting with 1.4.1 the published version and this changelog are aligned again.

## [1.4.1] - 2026-07-10
### Fixed
- **Organic installs no longer fail install-response decoding.** Older Core servers return `deepLinkData: {}` (empty object) instead of `null` for unattributed installs; decoding that threw on the required `shortCode`, failing the whole `InstallResponse` decode and silently skipping deep link callback registration for that session. `InstallResponse` now treats any undecodable `deepLinkData` payload as organic (`nil`). The Core server was also fixed to return `null`, but the SDK stays tolerant of older self-hosted servers.
